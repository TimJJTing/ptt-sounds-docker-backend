...
class WavFileWriter:
    ...
    def __init__(self, filename='sound.wav', framerate=11025, nchannels=1):
    #def __init__(self, filename='sound.wav', framerate=11025):
        ...
        # 2 bytes
        self.sampwidth = 2
        # if sampwidth is 2 bytes, then it is 16 bits
        self.bits = self.sampwidth * 8
        # max Volume, if bits=16, then max Volume is 2^(16-1)-1
        # i.e. possible value ranges from -32767 to 32768
    ...
    def write_stereo(self, lwave, rwave):
        """Writes a stereo wave.

        lwave: Wave
        rwave: Wave
        """
        lzs = lwave.quantize(self.bound, self.dtype)
        rzs = rwave.quantize(self.bound, self.dtype)
        
        # write bytes
        for frame in range(len(lzs)):
            l = lzs[frame]
            r = rzs[frame]
            self.fp.writeframes(struct.pack('<hh', l, r ))
    ...
    def close(self, duration=0):
        ...
        if duration:
            if self.nchannels == 1:
                # write silence signal to both left and right 
                # self.write(rest(duration), rest(duration))
...
class BeatSeq():
    """Represents a signal with varying frequency based on the given input nparray."""
    
    def __init__(self, varocts, amp=1.0, basefreq=440):
        """Initializes a beat sequence.
        
        varocts: numpy array. An array of varying octaves
        basefreq: float. Base frequency
        
        """
        self.freqbeats = 2**(varocts+np.log2(basefreq))
        self.amp = amp

    def make_wave(self, duration=1.0, framerate=11025, *args, **kwargs):
        """Make the wave of a sequence of beats.

        duration: float. Duration of this sequence
        aaenv: AdsrAmpEnvelope. Amp envelope for a beat. Optional
        
        returns: Wave
        """
        # adsr envelope
        if not kwargs.get("aaenv"):
            aaenv = AdsrAmpEnvelope(adr=[0.1,0.1,0.1], suslv=0, inc_type='lin')
        else:
            aaenv = kwargs.get("aaenv")
        # duration of a beat
        beatsdur = duration / len(self.freqbeats)
        
        beatseq = None
        for freqbeat in self.freqbeats:
            beatsignal = SinSignal(freq=freqbeat, amp=self.amp, offset=0)
            beatwave = beatsignal.make_wave(duration=beatsdur)
            # adsr
            if not beatseq:
                beatseq = aaenv.evaluate_wave(beatwave)
            else:
                beatseq = beatseq | aaenv.evaluate_wave(beatwave)
        # len(beatseq.ts) must be consistant = framerate * duration // 1
        # if not, make it be
        samplelen = int(framerate*duration)
        if len(beatseq.ys) > samplelen:
            beatseq.ys = beatseq.ys[:samplelen]
            beatseq.ts = np.arange(len(beatseq.ys)) / framerate
        elif len(beatseq.ys) < samplelen:
            beatseq.ys = np.concatenate([beatseq.ys, np.zeros(samplelen-len(beatseq.ys))])
            beatseq.ts = np.arange(len(beatseq.ys)) / framerate
...
class AdsrAmpEnvelope:
    """An ADSR envelope generator for amp.
    """
    def __init__(self, adr=[0.1,0,0.1], suslv=1, inc_type='exp', exp_const=1.5):
        """
        adr: list. duration percentage of attack, decay, and release. The rest percentage is sustain.
        suslv: float. Level param. x% of attack
        wave: Wave. the wave input to be modified
        inc_type: string. Growth/Decay linearly or exponentially. Avaliable options are "lin" and "exp"
        exp_const: float. Exponential growth/decay constant. The bigger the more rapid.
        return: ys that can be multiplied with the ys of env's input.
        """
        # param validations
        if sum(adr) > 1 or sum(adr) < 0:
            raise ValueError('adsr_envelope: The sum of attack, decay, and release must be larger than the duration.')
        if suslv > 1 or suslv < 0:
            raise ValueError("Sustain must be <= 1 and >= 0.")
        if inc_type not in ['lin', 'exp']:
            raise AttributeError('adsr_envelope: The inc_type must be either "lin" or "exp".')
    ...
    def evaluate_wave(self, wave):
        """ Takes a wave object and modify its amp with adsr params.
            wave: Wave. The wave object to be modified
        """
        return self._evaluate(wave.ts, wave.ys, wave.framerate)
    
    def _evaluate_amps(self, ts, framerate):
        """ Given ts, evaluate the ADSR envelope function piecewise.
        """
        duration = len(ts) / framerate
        dec_st = duration * self.atk
        sus_st = dec_st + duration * self.dec
        rel_st = sus_st + duration * self.sus
        # attack stage: t <= self.decay_start: t * self.max / self.attack
        # decay stage: self.decay_start < t <= self.sustain_start: ((self.sustain-1)*t+self.decay-self.attack*(self.sustain-1))*self.max/self.sustain
        # sustain stage: self.sustain_start < t <= self.release_start: self.max * self.sustain
        # release stage: t <= self.duration: -self.max*self.sustain/self.release*(t-self.duration)
        if self.inc_type == 'lin':
            fatk = lambda t: t/(duration*self.atk)
            fdec = lambda t: ((self.suslv-1)*t+self.dec*duration-self.atk*duration*(self.suslv-1))/(self.dec*duration)
            fsus = lambda t: self.suslv
            frel = lambda t: -self.suslv/(self.rel*duration)*(t-duration)
            fdef = lambda t: 0
            
        elif self.inc_type == 'exp':
            # definition: https://www.dsprelated.com/freebooks/mdft/Exponentials.html
            fatk = lambda t: 1-np.exp(-self.exp_const/self.atk*duration*t)
            fdec = lambda t: (1-self.suslv)*((1/np.exp(self.exp_const/self.dec*duration*(t-self.atk*duration)))+1)
            fsus = lambda t: self.suslv
            frel = lambda t: self.suslv/np.exp(self.exp_const/self.rel*duration*(t-rel_st))
            fdef = lambda t: 0
            
        return np.piecewise(ts,
                            [(0<=ts)*(ts<=dec_st), (dec_st<ts)*(ts<=sus_st), (sus_st<ts)*(ts<=rel_st), (rel_st<ts)*(ts<=duration)],
                            [fatk, fdec, fsus, frel, fdef])
        
    def _evaluate(self, ts, ys, framerate):
        """ Given ts, evaluate the ADSR envelope function piecewise.
        """
        new_ys = self._evaluate_amps(ts, framerate) * ys
        return Wave(new_ys, ts, framerate)
    
    def plot(self, duration, framerate):
        """ Plot the waveform of the envelope.
        """
        x = np.arange(0, duration, 1/framerate)
        y = self._evaluate_amps(x, framerate)
        plt.plot(x, y)
        plt.ylim(-0.1, 1.1)
        plt.xlim(-0.1, x.max()+0.1)
        plt.show()
...
# Stereo wave
class StereoWave:
    """bind two (mono) Waves as a Stereo.

    """
    #def __init__(self, lwave, rwave, ys, ts=None, framerate=None):
    def __init__(self, lwave, rwave):
        """Initializes the wave.
        ...
        framerate: samples per second
        """
        if lwave.framerate != rwave.framerate:
            raise ValueError('the framerate of lwave must be equal to the framerate of rwave.')
        
        if len(lwave) != len(rwave):
            # TODO: remove this when todos below have been finished
            raise ValueError('length of waves do not match')

        if lwave.ts[0] != rwave.ts[0]:
            # TODO: concatenate silence signal
            raise ValueError('time stamp not match (ts[0])')
        if lwave.ts[-1] != rwave.ts[-1]:
            # TODO: concatenate silence signal
            raise ValueError('time stamp not match (ts[-1])')
        
        self.lwave = lwave
        self.rwave = rwave
        self.framerate = lwave.framerate
        # ts = np.arange(min(self.lwave.ts[0],self.rwave.ts[0]), max(self.lwave.ts[-1], self.rwave.ts[-1]), 1/self.framerate)
    ...
    #     return self.ts[-1]
    def __add__(self, other):
        """Adds two stereowaves elementwise.

        other: StereoWave

        returns: new StereoWave
        """
        if other == 0:
            return self
        ...
        return StereoWave(self.lwave+other.lwave, self.rwave+other.rwave)

    __radd__ = __add__

    def __or__(self, other):
        """Concatenates two stereo waves.
        ...
        """
        if self.framerate != other.framerate:
            raise ValueError('StereoWave.__or__: framerates do not agree')

        return StereoWave(self.lwave|other.lwave, self.rwave|other.rwave)
    
    def __mul__(self, other):
        """Multiplies two stereowaves elementwise.
    ...
    def write(self, filename='sound.wav'):
        """Write a stereowave file.

        filename: string
        """
        print('Writing', filename)
        wfile = WavFileWriter(filename, self.framerate, nchannels=2)
        wfile.write_stereo(self.lwave, self.rwave)
        wfile.close()
...
# class VarChirp(Chirp):
#     """Represents a signal with variable frequency."""
    
#     def __init__(self, varfreqs, varamps):
#         """Initializes a varying chirp.

#         start: float frequency in Hz
#         end: float frequency in Hz
#         amp: float amplitude, 1.0 is nominal max
#         """
#         self.varfreqs = varfreqs
#         self.varamp = varamps
...
#         returns: float wave array
#         """
#         parts = len(self.varfreqs)
#         i=0
#         while i < parts:
#             start, end = np.log10(self.varfreqs[i]), np.log10(self.varfreqs[i+1])
#             freqs = np.concatenate(freqs, np.logspace(start, end, len(ts)/parts-1))
#             i+=1
#         return self._evaluate(ts, freqs)
